...
dfsCutSearch([(CurrentNode, From) | TQueue], N, ChosenPositions, SidesOfBoardFoundSoFar, UsedEdges, VisitedNodes):-
    member( (From, CurrentNode), UsedEdges), %list with member is faster than ord_set here since these are often small lists. I tested it!
...
dfsCutSearch( [ (CurrentNode, _From) | _TQueue], N, _ChosenPositions, 1 , _UsedEdges, _VisitedNodes):-
    isOnSideOfBoard(CurrentNode,N),
    !,
    fail.
...
dfsCutSearch([(CurrentNode, _From) | _TQueue], _N, _ChosenPositions, _SidesOfBoardFoundSoFar, _UsedEdges, VisitedNodes):-
...
dfsCutSearch([(CurrentNode, From) | TQueue], N, ChosenPositions, SidesOfBoardFoundSoFar, UsedEdges, VisitedNodes):-
    calcNewSidesOfBoardFoundSoFar(CurrentNode, N, SidesOfBoardFoundSoFar, NewSidesOfBoardFoundSoFar),
    findAllPositionsToExplore(ChosenPositions, CurrentNode, Neighbors),
    append(TQueue, Neighbors, Queue),
    dfsCutSearch(Queue, N, ChosenPositions, NewSidesOfBoardFoundSoFar, [(CurrentNode,From)|UsedEdges], [CurrentNode|VisitedNodes]),
    !.
